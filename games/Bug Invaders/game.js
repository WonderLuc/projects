function preload() {
  ...
  function create() {
    ...
    gameState.enemies = this.physics.add.group();
      ...
      for(let xVal =1; xVal < 9; xVal++){
        gameState.enemies.create(50*xVal, 50*yVal, 'bug' +(Math.round(Math.random()*2) + 1)).setScale(.6).setGravityY(-200); 
      }
    }
    
   
    
    //Add pellets and their behavior
    ...
    this.physics.add.collider(gameState.enemies, gameState.bugRepellent,(bug,repellent)=>{
     ...
     this.physics.add.collider(gameState.enemies,gameState.player,()=>{
      gameState.active = false;                                     gameState.enemyVelocity = 1;                                   this.physics.pause();
      this.add.text(210,250, 'Game Over \n Click to restart', { fontSize: '15px', fill: '#000' }); 
    });
  }
  ...
  function update() {
      if (gameState.active) {
          ...
          // logic for winning condition and enemy movements 
      if(numOfTotalEnemies() === 0){
        gameState.active = false;
        this.physics.pause();
        this.add.text(210,250, 'You Win!', { fontSize: '15px', fill: '#000' });
